...
const App: React.FC = () => {
  ...
  const handleUpsertDayData = async (dateKey: string, data: DayData) => {
    ...
    await supabase
      .from('day_data')
      .upsert(
        { user_id: userIdForDb, date: dateKey, data: dataForDb },
  ...
  const handleNavigateToRevenue = (
    ...
    let initialState:
      | { viewMode: 'daily' | 'analysis'; dateRange?: { start: string; end: string } }
  ...
  const renderView = () => {
    ...
    switch (view) {
      ...
      case 'team-control':
        return (
          <TeamControlPage users={users} onViewUserTrends={handleViewUserTrends} />
        );
