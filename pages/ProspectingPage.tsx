...
const ProspectingPage: React.FC<ProspectingPageProps> = ({ allData, onDataChange, selectedDate, onDateChange, onAddHotLead, onAddWin, handleSetAppointment, hotLeads }) => {
    ...
    const handleContactChange = (index: number, field: keyof Omit<Contact, 'id' | 'prospecting'>, value: string) => {
    ...
    const handleCSVImport = (importedData: Array<Partial<Contact>>) => {
        ...
        for (const data of importedData) {
            const emptyIndex = newContacts.findIndex(c => !c.name && !c.phone && !c.email);
            if (emptyIndex !== -1) {
     ...
     const handleQuickSetAppointment = (data: { name: string, phone: string, email: string, date: string, time: string, interestLevel: number }) => {
        ...
        handleSetAppointment({ client: data.name, lead: 'Prospecting Page', time: data.time }, appointmentDate);
        ...
        const emptyIndex = newContacts.findIndex(c => !c.name);
         if (emptyIndex !== -1) {
            newContacts[emptyIndex] = {
                ...newContacts[emptyIndex],
                name: data.name, phone: formatPhoneNumber(data.phone), email: data.email,
                date: new Date().toISOString().split('T')[0], prospecting: { SA: true }, interestLevel: data.interestLevel
            };
            updateCurrentData({ prospectingContacts: newContacts });
         }
    };
    
    const handleQuickAddToHotLeads = async (data: { name: string, phone: string, email: string, interestLevel: number }) => {
        const newContactData = {
            name: data.name, date: new Date().toISOString().split('T')[0], phone: formatPhoneNumber(data.phone), email: data.email,
            ...
            <div className="lg:col-span-2">
                <div className="bg-brand-light-card dark:bg-brand-navy p-4 rounded-lg border border-brand-light-border dark:border-brand-gray h-full">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-xl font-bold text-brand-light-text dark:text-white">Prospecting List - {selectedDate.toLocaleDateString()}</h2>
                    ...
                    <div className="overflow-x-auto max-h-[28rem] overflow-y-auto">
                        <table className="w-full text-sm text-left table-fixed">
                            <thead className="bg-brand-light-bg dark:bg-brand-gray/50 text-xs uppercase text-gray-500 dark:text-gray-400 sticky top-0 z-10">
                                <tr>
                                    <th className="p-2 w-10">#</th><th className="p-2 w-1/6">Name</th><th className="p-2 w-1/6">Date</th>
                                    <th className="p-2 w-1/6">Phone</th><th className="p-2 w-1/5">Email</th><th className="p-2 w-16 text-center">Actions</th>
                                    <th className="p-2 w-[240px] text-center">Codes</th>
                                </tr>
                            </thead>
                            <tbody>
                                {currentData.prospectingContacts.map((contact, index) => (
                                    <tr key={contact.id} className="border-b border-brand-light-border dark:border-brand-gray text-brand-light-text dark:text-white">
                                        <td className="p-2 text-gray-500 dark:text-gray-400 text-center">{index + 1}</td>
                                        <td><input type="text" value={contact.name} onChange={e => handleContactChange(index, 'name', e.target.value)} className="w-full bg-transparent p-1 focus:outline-none focus:bg-brand-light-bg dark:focus:bg-brand-gray/50 rounded dark:text-white" /></td>
                                        <td className="p-1"><DatePicker value={contact.date} onChange={value => handleContactChange(index, 'date', value)} /></td>
                                        <td><input type="tel" value={contact.phone} onChange={e => handleContactChange(index, 'phone', e.target.value)} className="w-full bg-transparent p-1 focus:outline-none focus:bg-brand-light-bg dark:focus:bg-brand-gray/50 rounded dark:text-white" /></td>
                                        <td><input type="email" value={contact.email} onChange={e => handleContactChange(index, 'email', e.target.value)} className="w-full bg-transparent p-1 focus:outline-none focus:bg-brand-light-bg dark:focus:bg-brand-gray/50 rounded dark:text-white" /></td>
                                        <td className="p-2 text-center">
                                            <button onClick={() => handleMarkAsHotLead(contact)} disabled={!contact.name} className="text-2xl disabled:text-gray-600 dark:disabled:text-gray-700 disabled:cursor-not-allowed filter grayscale disabled:grayscale-0 hover:grayscale-0 transition-all" title="Mark as Hot Lead">🔥</button>
                                        </td>
                                        <td className="p-2">
                                            <div className="flex items-center justify-center space-x-1">
                                                {prospectingCodes.map(code => (
                                                    <button key={code} onClick={() => handleProspectingChange(index, code)} title={prospectingCodeDescriptions[code]}
                                                        className={`w-7 h-7 flex items-center justify-center rounded text-xs font-mono transition-colors ${
                                                            contact.prospecting[code] ? (code === 'SA' ? 'bg-brand-blue text-white' : 'bg-brand-red text-white') : 'bg-gray-200 dark:bg-brand-gray text-gray-500 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-brand-gray/50'
                                                        }`}>
                                                        {code}
                                                    </button>
                                                ))}
                    ...
                    <div className="mt-4 pt-2 border-t border-brand-light-border dark:border-brand-gray">
                        <h4 className="text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-2">Code Legend</h4>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 text-xs">
                            {prospectingCodes.map(code => (<div key={code} className="flex items-center"><span className="font-mono font-bold text-brand-light-text dark:text-white mr-2">{code}</span><span className="text-gray-500 dark:text-gray-400">{prospectingCodeDescriptions[code]}</span></div>))}
                        </div>
                    </div>
                </div>
            </div>
