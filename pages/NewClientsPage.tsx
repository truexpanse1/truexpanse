...
const ClientCard: React.FC<{
  ...
  onClick: () => void;
  userColor?: string;
}> = ({ client, onClick, userColor = '#6b7280' }) => (
  <div
    onClick={onClick}
...
// --- helper to avoid .split() on null/undefined ---
const getRepLabel = (rep: User): string => {
  // Prefer name; if missing, fall back to email; finally generic label.
  const base =
    (rep.name && rep.name.trim()) ||
    (rep.email && rep.email.trim()) ||
    'Rep';

  // base is guaranteed to be a non-empty string here
  return base.split(' ')[0];
};
...
const NewClientsPage: React.FC<NewClientsPageProps> = ({
  ...
  const filteredClients = useMemo(() => {
    ...
    return repFiltered.filter((client) => {
      const clientPhoneNumeric =
        client.phone?.replace(/[^\d]/g, '') || '';
      return (
        client.name?.toLowerCase().includes(lowercasedFilter) ||
        client.company?.toLowerCase().includes(lowercasedFilter) ||
        client.email?.toLowerCase().includes(lowercasedFilter) ||
        (numericSearchTerm &&
          clientPhoneNumeric.includes(numericSearchTerm))
      );
    });
  ...
  const handleSaveClient = (clientData: NewClient) => {
    onSaveClient(
      String(clientData.id).startsWith('manual-')
    ...
    importedData.forEach((client, index) => {
      const newClient: NewClient = {
        ...
        monthlyContractValue:
          Number(client.monthlyContractValue) || 0,
        initialAmountCollected:
          Number(client.initialAmountCollected) || 0,
        closeDate:
          client.closeDate &&
